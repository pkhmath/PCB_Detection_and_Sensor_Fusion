#******************************************************************************************************************************************
# TITLE     : ROUTINE_REV_MERGE_PXMTY_ALGO
...
#Reverse sensor fusion algorithm for pick-and-place machines
...
print('Version : 2019-06-12 19:06')

def rev_merge_pxmty_algo(inp_df,pxmty_df,inp_binry_sig,THRESHOLD,HIGH_END):
    ...
    spike_df  = pxmty_df.query('data_proximity == 1').loc[:,['timestamp']]
    ...
    while (END_FLAG == 0):
        ...
        if (i < 0):                     #already reached start of dataframe
            END_FLAG = 1
        elif (i == 0):                  #first entry of the dataframe  
            merge_flag = 0
        else:    
            if (inp_df.weightage[i] <= THRESHOLD):
                ...
                if (spk_i_num > 0): #valid and first part of pcb
                    if (inp_df.weightage[i-1] <= THRESHOLD):
                        ...
                        if (spk_iml_num == 0): #not a first part of pcb processing
        ...
        if (END_FLAG == 0):
            if (merge_flag == 1):
                arvl_index  = inp_df.arvl_index[i-1]
                arvl_tmstmp = inp_df.arvl_tmstmp[i-1]
                dptr_index  = inp_df.dptr_index[i]
                dptr_tmstmp = inp_df.dptr_tmstmp[i]
                no_of_parts = inp_df.no_of_parts[i] + 1
                proc_dur    = inp_df.proc_dur[i-1] + inp_df.proc_dur[i]
                maint_dur   = inp_df.maint_dur[i] + pd.Timedelta(pd.Timestamp(inp_df.arvl_tmstmp[i]) - pd.Timestamp(inp_df.dptr_tmstmp[i-1])).total_seconds()
                weightage   = inp_df.weightage[i-1] + inp_df.weightage[i]
                
                #Indication for maintenance duration
                STR = inp_df.dptr_index[i-1]
                END = inp_df.arvl_index[i]
    ...
    out_df = out_df.sort_values(by='arvl_tmstmp')    #becuase we have done merging in reverse order
